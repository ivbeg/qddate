...
PAT_EN_DAY_NUMERIC = (Word(nums, min=1, max=2).setResultsName("day") +
...
BASE_DATE_PATTERNS = {
    ...
    Literal("-").suppress() + Word(nums, exact=4).setResultsName("year"),
    "pat:date:yyyy-m-d":
    Word(nums, exact=4).setResultsName("year") + Literal("-").suppress() +
    Word(nums, min=1, max=2).setResultsName("month") +
    Literal("-").suppress() + Word(nums, min=1, max=2).setResultsName("day"),
    "pat:date:yyyy.m.d":
    Word(nums, exact=4).setResultsName("year") + Literal(".").suppress() +
...
BASE_TIME_PATTERNS = {
    "pat:time:minutes":
    Word(nums, exact=2).setResultsName("hour") + Literal(":").suppress() +
    Word(nums, exact=2).setResultsName("minute"),
    "pat:time:full":
    Word(nums, exact=2).setResultsName("hour") + Literal(":").suppress() +
    Word(nums, exact=2).setResultsName("minute") + Literal(":").suppress() +
    Word(nums, exact=2).setResultsName("second") + Optional(
        Literal("+").suppress() +
        Word(nums, min=3, max=4).setResultsName("timezone")),
}
...
BASE_PATTERNS_EN = {
    ...
    oneOf(ENG_WEEKDAYS_SHORT, caseless=True).setParseAction(
...
PATTERNS_EN = [
    ...
    },
    {
        ...
        Optional("-").suppress() +
        BASE_PATTERNS_EN["pat:eng:months:short"].setResultsName("month") +
        Optional("-").suppress() + Word(nums, exact=2).setResultsName("year"),
        "length": {
            "min": 10,
            "max": 10
        },
